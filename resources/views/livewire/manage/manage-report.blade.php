        ...
        <div class="mt-4 mx-auto max-w-6xl">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
                ...
                <div class="col-span-2 md:col-span-2">
                    <x-native-select searchable label="Select Department" placeholder="Select one department" wire:model="department">
            ...
            <div class="mt-2 ">
                <div class="px-6 ">
                    <table class="w-full divide-y divide-gray-300">
                        <thead>
                          <tr>
                              <th scope="col" class=" px-3 py-2.5 text-left text-sm font-semibold text-gray-900 ">Name</th>
                              <th scope="col" class=" px-3 py-2.5 text-left text-sm font-semibold text-gray-900 ">Department</th>
                              <th scope="col" class=" px-3 py-2.5 text-left text-sm font-semibold text-gray-900 ">Position</th>
                              <th scope="col" class=" px-3 py-2.5 text-left text-sm font-semibold text-gray-900 ">Course</th>
                            <th scope="col" class=" px-3 py-2.5 text-center text-sm  font-semibold text-gray-900">Morning in & out</th>
                            <th scope="col" class=" px-3 py-2.5 text-center text-sm  font-semibold text-gray-900">Afternoon in & out</th>
                        ...
                        <tbody class=" bg-white">
                            ...
                            <tr>
                                ...
                                <td class="whitespace-nowrap px-3 py-2.5 text-sm text-gray-600">{{$item->department_name ?? ''}}</td>
                                  <td class="whitespace-nowrap px-3 py-2.5 text-sm text-gray-600">{{$item->role_name ?? ''}}</td>
                                  <td class="whitespace-nowrap px-3 py-2.5 text-sm text-gray-600">{{$item->course_name ?? ''}}</td>
                                  <td class="whitespace-nowrap text-center px-3 py-2.5 text-sm text-gray-600">
                           
                                    {{ optional($item->login)->morning_in ? \Carbon\Carbon::parse($item->login->morning_in)->format('h:i:s A') : 'NONE' }}
                                    |
                                    {{ optional($item->login)->morning_out ? \Carbon\Carbon::parse($item->login->morning_out)->format('h:i:s A') : 'NONE' }}
                                    
                                </td>
                                <td class="whitespace-nowrap text-center px-3 py-2.5 text-sm text-gray-600">
                                    {{ optional($item->login)->noon_in ? \Carbon\Carbon::parse($item->login->noon_in)->format('h:i:s A') : 'NONE ' }}
                                    |
                                    {{ optional($item->login)->noon_out ? \Carbon\Carbon::parse($item->login->noon_out)->format('h:i:s A') : 'NONE' }}
